...
initial_error = getSpeedError(guess)

print("Initial error: ", initial_error)

def getSpeedErrorPercentage(params):
    new_sim_error = getSpeedError(params)
    percent_error = abs((initial_error - new_sim_error)/initial_error)
    print("error percetange: ", percent_error)
    return percent_error

error_constraint = NonlinearConstraint(getSpeedErrorPercentage,0,0.10,keep_feasible=True)
